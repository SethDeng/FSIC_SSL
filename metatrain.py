...
PRETRAIN_DIR='/root/FSL/FSL_CL/pretrain_model/miniimagenet'
# Dataset path
...
parser.add_argument('-random_val_task', type=int,default=0, help='random samples tasks for validation at each epoch')
...
parser.add_argument('-con_momentum', type=float, default=0.999)
parser.add_argument('-temparature', type=float, default=0.07)
parser.add_argument('-mlp_dim', type=int, default=128)
parser.add_argument('-beta', type=float, default=1.0)
parser.add_argument('-queue_len', type=int, default=65500)
...
model = DeepEMD_CL(args)
model = load_model_cl(model, args.pretrain_dir)
...
train_loader = DataLoader(dataset=trainset, batch_sampler=train_sampler, num_workers=48, pin_memory=True)

val_sampler = CategoriesSampler(valset.label, args.val_episode, args.way, args.shot + args.query)
val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=48, pin_memory=True)
...
label = torch.arange(args.way, dtype=torch.int8).repeat(args.query)#01234
...
for epoch in range(1, args.max_epoch + 1):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        data, con_im_1, con_im_2, _ = [_.cuda() for _ in batch]
        ...
        loss_emd = F.cross_entropy(logits, label)

        #########################################
        # Contrastive Loss
        #########################################

        model.module.mode = 'contrastive'
        logits_con = model([con_im_1, con_im_2])
        label_con = torch.zeros(logits_con.shape[0], dtype=torch.long).cuda()
        loss_con = F.cross_entropy(logits_con, label_con)
        
        #########################################
        # Total Loss
        #########################################

        loss = args.beta * loss_emd + (1.0 - args.beta) * loss_con
        # loss = loss_emd

        acc = count_acc(logits, label)
        acc_con = count_acc(logits_con, label_con)
    ...
    print('Contrastive Loss: ', cl.item())
    print('EMD Acc: ', ta.item())
    print('Contrastive Acc: ', ca.item())
    ...
    if va >= trlog['max_acc']:
        ...
        max_acc_list.append(va)
        max_acc_epoch_list.append(epoch)
        max_acc_path_list.append(osp.join(args.save_path, 'epoch-%d'%epoch + '.pth'))
...
print('********* First Test: ', args.test_episode, ' *********')

for max_iter in range(len(max_acc_path_list)):
    test_start_time = time.time()
    test_acc_record = np.zeros((args.test_episode,))
    model.load_state_dict(torch.load(max_acc_path_list[max_iter])['params'])
    model.eval()
    
    # data prepare
    testset = Dataset('test', args)
    sampler = CategoriesSampler(testset.label, args.test_episode, args.way, args.shot + args.query)
    test_loader = DataLoader(testset, batch_sampler=sampler, num_workers=48, pin_memory=True)
    ...
    result_list.append('Epoch {}, Val Acc {:.4f}'.format(max_acc_epoch_list[max_iter], max_acc_list[max_iter]))
    result_list.append('Test Acc {:.4f} + {:.4f}'.format(m, pm))

    print(result_list[-2])
    print(result_list[-1])
    print('This epoch takes %d seconds'%(time.time() - test_start_time),'\nTest phase still need %.2f hour to finish'%((time.time() - test_start_time)*(len(max_acc_path_list) - max_iter - 1)/3600))
